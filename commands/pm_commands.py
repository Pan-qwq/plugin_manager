...
命令列表：
  /pm list                        — 列出所有已加载插件
  /pm check [插件名]               — 检查 GitHub + 市场版本与新订阅
...
class PMCommand(BaseCommand):
    ...
    def _get_plugin_source_info(self, plugin_name: str) -> dict[str, Any]:
        ...
        if source == "market":
            # 市场插件：auto_update 跟随 core.toml 的 auto_update_mfp
            core_cfg = self._get_core_config()
            if core_cfg and hasattr(core_cfg, "plugin_market"):
                auto_update = getattr(core_cfg.plugin_market, "auto_update_mfp", False)
            # update_policy 从本插件配置读取
            pm_cfg = self._get_config()
            if pm_cfg and hasattr(pm_cfg, "market"):
                update_policy = getattr(pm_cfg.market, "default_update_policy", "silent")
        elif source == "github":
            # GitHub 插件：默认开启 auto_update，策略从配置读取
            auto_update = True
            pm_cfg = self._get_config()
            if pm_cfg and hasattr(pm_cfg, "github"):
                update_policy = getattr(pm_cfg.github, "default_update_policy", "notify")

        return {"source": source, "auto_update": auto_update, "update_policy": update_policy}
    ...
    @staticmethod
    def _get_core_config() -> Any:
        """获取框架核心配置（含 plugin_market 节）。"""
        try:
    ...
    @staticmethod
    def _get_core_plugins_dir() -> str:
        """获取插件目录路径。"""
        try:
            cfg = PMCommand._get_core_config()
            if cfg:
                return cfg.bot.plugins_dir
        except Exception:
            pass
        # 降级：从本插件路径推断
        path = plugin_api.get_plugin_path("plugin_manager")
    ...
    async def _refresh_market_cache(self, force: bool = False, config: Any = None) -> dict[str, Any]:
        """刷新市场订阅缓存。过期或强制时发起 API 调用。
        返回 market_cache dict（含 subscriptions / fetched_at）。
        """
        settings = self._load_plugin_settings()
        ...
        # 读取核心配置中的 [plugin_market] 节
        core_cfg = self._get_core_config()
        if not core_cfg or not hasattr(core_cfg, "plugin_market"):
            logger.warning("核心配置无 [plugin_market] 节，跳过市场缓存")
            fallback = {"subscriptions": [], "fetched_at": now}
            settings["market_cache"] = fallback
            self._save_plugin_settings(settings)
            return fallback

        pm = core_cfg.plugin_market
        if not getattr(pm, "enabled", False):
            logger.debug("市场同步未启用，跳过")
            fallback = {"subscriptions": [], "fetched_at": now}
            settings["market_cache"] = fallback
            self._save_plugin_settings(settings)
            return fallback

        base_url = getattr(pm, "base_url", "").rstrip("/")
        user_id = getattr(pm, "user_id", "")
        token = getattr(pm, "access_token", "")
        timeout = getattr(pm, "timeout", 20.0)
        if not base_url or not user_id or not token:
            logger.warning("市场配置不完整（缺 base_url/user_id/access_token）")
            fallback = {"subscriptions": [], "fetched_at": now}
            settings["market_cache"] = fallback
            self._save_plugin_settings(settings)
            return fallback
        ...
        try:
            ...
            async with httpx.AsyncClient(verify=False, timeout=timeout) as client:
                resp = await client.get(
                    f"{base_url}/api/v1/machine/authors/{user_id}/subscriptions",
                    headers=headers,
                )
                ...
                result = {"subscriptions": subs, "fetched_at": now}
                settings["market_cache"] = result
                self._save_plugin_settings(settings)
                logger.info(f"市场订阅缓存已刷新: {len(subs)} 条")
                # 自动安装缺失的订阅插件
                await self._auto_install_missing_subs(subs, config)
                return result
        except httpx.HTTPStatusError as e:
            logger.warning(f"市场 API HTTP 错误 [{e.response.status_code}]: {e}")
        except httpx.RequestError as e:
            logger.warning(f"市场 API 请求失败: {e}")
        except Exception as e:
            logger.warning(f"刷新市场缓存异常: {e}")
    ...
    async def _auto_install_missing_subs(
        self, subs: list[dict[str, str]], config: Any
    ) -> None:
        """自动安装已订阅但本地缺失的市场插件。"""
        core_cfg = self._get_core_config()
        if not core_cfg or not hasattr(core_cfg, "plugin_market"):
            return
        pm = core_cfg.plugin_market
        if not getattr(pm, "auto_install_subscribed_missing", False):
            return
        loaded = set(plugin_api.list_loaded_plugins())
        installed_count = 0
        for sub in subs:
            name = sub.get("name", "") or sub.get("plugin_id", "")
            if not name:
                continue
            if name in loaded:
                continue
            # 也检查磁盘上是否有（即使未加载）
            plugins_dir = Path(self._get_core_plugins_dir())
            if (plugins_dir / name).exists() or (plugins_dir / f"{name}.mfp").exists():
                continue
            logger.info(f"自动安装缺失的订阅插件: {name}")
            ok = await self._do_market_install(sub)
            if ok:
                installed_count += 1
                loaded.add(name)
        if installed_count:
            logger.info(f"自动安装了 {installed_count} 个缺失的订阅插件")

    async def _do_market_install(self, sub: dict[str, str]) -> bool:
        """从市场下载并安装一个插件。sub 含 plugin_id / name / latest_version。"""
        plugin_id = sub.get("plugin_id", "")
        name = sub.get("name", "") or plugin_id
        version = sub.get("latest_version", "")
        if not plugin_id:
            return False
        ...
        try:
            async with httpx.AsyncClient(verify=False, timeout=timeout) as client:
                ...
                install_resp = await client.get(
                    f"{base_url}/api/v1/plugins/{plugin_id}/install",
                    headers=headers,
                )
                install_resp.raise_for_status()
                install_data = install_resp.json()
                version_info = install_data.get("version", install_data)
                download_url = version_info.get("asset_download_url", "")
                if not download_url:
                    logger.warning(f"市场插件 {name} 无下载地址")
                    return False
            ...
            mfp_path = plugins_dir / f"{name}.mfp"
            mfp_path.write_bytes(content)
            logger.info(f"市场插件已下载: {mfp_path} ({len(content)} bytes)")

            # 4. 尝试加载
            try:
                ok = await plugin_api.load_plugin(str(mfp_path))
                if ok:
                    ...
                    if not source_info.get("auto_update", False):
                        ...
                        source_info["update_policy"] = (
                            source_info.get("update_policy", "silent")
                        )
                        self._set_plugin_source_info(name, source_info)
                    return True
                else:
                    logger.warning(f"市场插件 {name} 下载但加载失败")
                    return False
            except Exception as e:
                logger.error(f"市场插件 {name} 加载异常: {e}")
                return False
        except Exception as e:
            logger.warning(f"市场安装 {name} 失败: {e}")
            return False
    ...
    async def _do_market_update(self, name: str, version: str, config: Any) -> tuple[bool, str]:
        """从市场更新指定的 .mfp 插件。返回 (success, message)。"""
        # 1. 自动备份
        backup_path = await self._create_backup(name)
        if backup_path:
            logger.info(f"更新前已备份: {backup_path}")
        # 2. 从 market_cache 查找 plugin_id
        settings = self._load_plugin_settings()
        market_cache = settings.get("market_cache", {})
        subs = market_cache.get("subscriptions", [])
        plugin_id = ""
        for sub in subs:
            if sub.get("name") == name or sub.get("plugin_id") == name:
                plugin_id = sub.get("plugin_id", "")
                break
        if not plugin_id:
            plugin_id = name
        ...
        core_cfg = self._get_core_config()
        if not core_cfg or not hasattr(core_cfg, "plugin_market"):
            return False, "核心配置无 [plugin_market] 节"
        pm = core_cfg.plugin_market
        base_url = getattr(pm, "base_url", "").rstrip("/")
        token = getattr(pm, "access_token", "")
        timeout = getattr(pm, "timeout", 20.0)
        if not base_url or not token:
            return False, "市场配置不完整"
        headers = {
            "Authorization": f"Bearer {token}",
            "User-Agent": "MoFox-PluginManager/1.0",
            "Accept": "application/json",
        ...
        try:
            async with httpx.AsyncClient(verify=False, timeout=timeout) as client:
                ...
                download_url = version_info.get("asset_download_url", "")
                if not download_url:
                    return False, f"{name}: 市场无下载地址"
                # 下载 .mfp
                dl_resp = await client.get(download_url, headers=headers)
                dl_resp.raise_for_status()
                content = dl_resp.content
            # 4. 覆盖到插件路径
            path = plugin_api.get_plugin_path(name)
            if not path:
                return False, f"{name}: 无法获取插件路径"
            dest = Path(path)
            if dest.is_file():
                dest.write_bytes(content)
            else:
                mfp_path = dest.parent / f"{name}.mfp"
                mfp_path.write_bytes(content)
            logger.info(f"市场插件 {name} 已下载更新 ({len(content)} bytes)")
        except Exception as e:
            logger.error(f"市场更新下载失败 {name}: {e}")
            return False, f"{name}: 市场下载失败 - {e}"
        # 5. 重载插件
        try:
            success = await plugin_api.reload_plugin(name)
            if success:
                return True, f"✅ {name} 已更新到 {version}（已热加载）"
            else:
                return False, f"⚠️ {name} 已下载到 {version}，但热加载失败，请手动重启 MoFox"
        except Exception as e:
            return False, f"⚠️ {name} 已下载到 {version}，但热加载异常: {e}"
    ...
    async def handle_default(self) -> tuple[bool, str]:
        ...
        help_text = (
            ...
            "━━━━━━━━━━━━━━━━\n"
            "• /pm list — 列出已加载插件\n"
            "• /pm check [插件] — 检查 GitHub 版本 + 市场版本 + 新订阅\n"
            "• /pm update <插件> — 更新 GitHub / 市场插件\n"
            "• /pm update --all — 批量更新（含市场）\n"
            "• /pm backup <插件> — 手动备份\n"
    ...
    async def handle_check(self, plugin_name: str = "") -> tuple[bool, str]:
        ...
        # ── 第三阶段：刷新市场缓存并检查市场订阅 ──
        market_cache = await self._refresh_market_cache(force=True, config=config)
        subs = market_cache.get("subscriptions", [])
        ...
        # GitHub 插件结果
        if to_check:
            lines.append("📦 GitHub 插件:")
            for r in check_results:
                lines.append(str(r) if isinstance(r, Exception) else r)
        # 无仓库信息的插件
        if no_repo:
            lines.append("")
            lines.append("⛔ 无仓库/市场信息:")
            for name in no_repo:
                lines.append(f"  • {name}")
        # 市场插件版本对比
        if market_plugins:
            lines.append("")
            lines.append("🏪 市场插件:")
            for name in market_plugins:
                local_ver = self._get_manifest_field(name, "version")
                remote_ver = ""
                for sub in subs:
                    if sub.get("name") == name or sub.get("plugin_id") == name:
                        remote_ver = sub.get("latest_version", "")
                        break
                if remote_ver and local_ver:
                    if local_ver == remote_ver:
                        lines.append(f"  ✅ {name}: 已是最新 ({local_ver})")
                    else:
                        lines.append(f"  🔄 {name}: {local_ver} → {remote_ver}")
                elif remote_ver:
                    lines.append(f"  📋 {name}: 本地={local_ver or '?'} 远程={remote_ver}")
                else:
                    lines.append(f"  📋 {name}: v{local_ver or '?'}（未在订阅列表中找到）")
        # 新订阅（已订阅但本地未安装的插件）
        if subs:
            loaded = set(plugin_api.list_loaded_plugins())
            new_subs = []
            ...
            if new_subs:
                lines.append("")
                lines.append(f"🆕 新订阅（未安装）:")
                for sub in new_subs:
                    n = sub.get("name", "") or sub.get("plugin_id", "")
                    v = sub.get("latest_version", "")
                    lines.append(f"  • {n} (v{v})")

        if not lines:
            lines.append("✅ 所有插件已是最新")
    ...
    async def handle_update(self, *args: str) -> tuple[bool, str]:
        ...
        if source == "market":
            # 市场插件更新
            await self._reply(f"🔄 正在检查 {plugin_name} 的市场更新…")
            market_cache = await self._refresh_market_cache(force=True, config=config)
            subs = market_cache.get("subscriptions", [])
            version = ""
            for sub in subs:
                if sub.get("name") == plugin_name or sub.get("plugin_id") == plugin_name:
                    version = sub.get("latest_version", "")
                    break
            if not version:
                await self._reply(f"❌ 无法获取 {plugin_name} 的市场版本")
                return False, "no market version"
            success, msg = await self._do_market_update(plugin_name, version, config)
            ...
            if source == "market":
                ...
                for sub in subs:
                    if sub.get("name") == name or sub.get("plugin_id") == name:
                        ...
                        break
                if not version:
                    results.append(f"⏭️ {name}: 市场无版本信息")
                    continue
                local_ver = self._get_manifest_field(name, "version")
